...
(function (global) {
  ...
  function settings(c) {
    ...
    c.innerHTML = head('설정', '보관소를 관리합니다.') +
      '<div class="card view-enter" style="padding:24px 26px;max-width:680px;margin-bottom:18px">' +
        ...
        '<p style="color:var(--text-faint);margin:10px 0 0;font-size:var(--fs-sm)">홈 화면 인사말에 쓰여요. 로그인하면 기기 간에 함께 동기화돼요.</p>' +
      '</div>' +
      '<div class="card view-enter" style="padding:24px 26px;max-width:680px;margin-bottom:18px">' +
      ...
      (function () {
        ...
        return '<div class="card" style="padding:24px 26px;max-width:680px;margin-bottom:18px">' +
          ...
          (u
            ? // ── 로그인됨: 실시간 동기화 ──
              '<p style="color:var(--text-dim);margin:0 0 16px;font-size:var(--fs-body)"><b style="color:var(--accent)">✓ 실시간 동기화 켜짐</b> — 꿈을 저장하면 자동으로 올라가고, 다른 기기에도 새로고침 없이 바로 반영돼요.</p>' +
              row('로그인 계정',
                ...
                '<span style="font-size:var(--fs-body)">' + esc(u.displayName || '') + (u.email ? ' <span style="color:var(--text-faint)">· ' + esc(u.email) + '</span>' : '') + '</span></span>') +
              row('', '<button class="btn sm danger" id="cloudLogout">로그아웃</button>')
            : // ── 미로그인 ──
              '<p style="color:var(--text-dim);margin:0 0 16px;font-size:var(--fs-body)">구글로 로그인하면, 그 계정의 꿈이 <b>모든 기기에서 실시간으로 같아져요.</b> 코드 입력도 새로고침도 필요 없어요.</p>' +
              (offline
                ? '<p style="color:var(--text-faint);margin:0;font-size:var(--fs-body)">⚠️ 지금은 오프라인이거나 로그인 모듈을 불러오지 못했어요. 인터넷 연결 후 다시 열어 주세요.</p>'
  ...
  function row(label, control) {
    return '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;gap:14px">' +
      '<span style="color:var(--text-dim);font-size:var(--fs-body)">' + esc(label) + '</span>' + control + '</div>';
  }
